...
│   └── Caching/
│       ├── CacheServiceProtocol<Item>   → Generic cache abstraction
│       └── FileSystemCacheService<Item>
...
- **`APIClient` + `BookService` separation** — The `APIClient` is a generic HTTP layer: give it an `Endpoint` and a `Decodable` type, it gives you back the decoded response. It knows nothing about books. `OpenLibraryBookService` uses `APIClient` internally and is the only place that knows about the Open Library API shape. If we added a second API (e.g. book details from a different service), `APIClient` is reused as-is.
...
The Open Library API is slow (2-4 seconds typical). A network-first strategy means users stare at a spinner on every launch. Instead, we show cached data instantly and refresh in the background:
...
3. Pull-to-refresh → always hits the network.
...
The app uses `NWPathMonitor` to detect connectivity changes in real time. When the device is offline or the user is viewing cached data, a non-intrusive animated banner appears at the top of the list. Pull-to-refresh re-attempts the network fetch, and the banner disappears automatically once fresh data arrives.

## Testing

35 tests across 5 suites, written with **Swift Testing** (`import Testing`, `@Test`, `#expect`) — Apple's modern testing framework. Chosen over XCTest for clearer diagnostics, `@Suite` organisation, and parameterised test support.
...
- **Work Model** — Author display formatting (single, multiple, none), cover URL construction, identity from API `key`, `Hashable` implies `Equatable` verification.
...
- **JSON Decoding** — Realistic API payloads, null/missing `cover_id`, multi-work response, round-trip encode/decode, and a parameterised test (`@Test(arguments:)`) that verifies all fixtures round-trip correctly.
...
- **No cache expiry.** The cache is indefinite. In production I'd add a TTL check (stale after 24 hours) and a background refresh strategy.
